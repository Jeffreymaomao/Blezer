...
2. **The process must be its own TCC "responsible process."** macOS attributes the Bluetooth request to the responsible process, not our binary. When spawned from another GUI app (e.g. a terminal, IDE, or Codex), that app is held responsible and crashes us if it lacks the key. **Launch via launchd** (`open "dist/Blezer.app" --args run ...` or a LaunchAgent) so the app is responsible for itself.
